...
import { badge, check, github, mail } from "../lib/Icons";
...
function ProfileModal() {
  ...
  // Destructure emailVerification from useUserContext
  const { user, updateUser, handlerUserInput, userState, changePassword, emailVerification } =
    useUserContext();
      ...
      >
        ...
        <div className="mt-4 relative flex justify-between">
          ...
          <div className="self-end flex items-center gap-2">
            {/* Changed from <button> to <span> to fix the nested button issue */}
            <span className="flex items-center gap-2 border-2 border-[#323232]/10 rounded-md py-1 px-3 text-xs font-medium text-[#323232]">
              {github} Github
            </span>
            {/* Conditional rendering for the verification status badge/button */}
            {user?.isVerified ? (
              // Changed from <button> to <span> for the non-interactive "Verified" status
              <span className="flex items-center gap-2 border-2 border-[#323232]/10 rounded-md py-1 px-3 text-xs font-medium text-[#323232]">
                {check} Verified
              </span>
            ) : (
              // This remains a button because it has an onClick handler
              <button
                className="flex items-center gap-2 border-2 border-red-500 rounded-md py-1 px-3 text-xs font-medium text-red-500"
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            // Calling closeModal() after the update to close the modal.
            closeModal();
          }}
        >
